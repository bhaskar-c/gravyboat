...
from gravyboat.apps.customer.utils import get_password_reset_url
from gravyboat.core.compat import get_user_model
from gravyboat.core.loading import (
    get_class, get_classes, get_model, get_profile_class)
from gravyboat.core.utils import safe_referrer
from gravyboat.views.generic import PostActionMixin

from . import signals

from gravyboat.apps.customer.mixins import PageTitleMixin, RegisterUserMixin
from gravyboat.apps.customer.utils import Dispatcher

from gravyboat.apps.customer.forms import EmailAuthenticationForm, EmailUserCreationForm, OrderSearchForm
from gravyboat.apps.customer.forms import PasswordChangeForm, ProfileForm, ConfirmPasswordForm
from gravyboat.apps.address.forms import UserAddressForm

from gravyboat.apps.order.models import Order
from gravyboat.apps.basket.models import Line, Basket
from gravyboat.apps.address.models import UserAddress
from gravyboat.apps.customer.models import Email
from gravyboat.apps.customer.models import ProductAlert
from gravyboat.apps.customer.models import CommunicationEventType
...
class AccountSummaryView(generic.RedirectView):
    ...
    that redirect can be configured via GRAVYBOAT_ACCOUNT_REDIRECT_URL), but
    ...
    """
    pattern_name = settings.GRAVYBOAT_ACCOUNTS_REDIRECT_URL
...
class LogoutView(generic.RedirectView):
    url = settings.GRAVYBOAT_HOMEPAGE
    permanent = False
    ...
    def get(self, request, *args, **kwargs):
        ...
        for cookie in settings.GRAVYBOAT_COOKIES_DELETE_ON_LOGOUT:
...
class ProfileDeleteView(PageTitleMixin, generic.FormView):
    ...
    active_tab = 'profile'
    success_url = settings.GRAVYBOAT_HOMEPAGE
...
class EmailHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.GRAVYBOAT_EMAILS_PER_PAGE
...
class OrderHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.GRAVYBOAT_ORDERS_PER_PAGE
    model = Order
...
class AddressListView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.GRAVYBOAT_ADDRESSES_PER_PAGE
