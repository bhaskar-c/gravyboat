...
def inherit_app_config(local_app_path, app_package, app_label):
    ...
    create_file(
        ...
        "from gravyboat.apps.{app_label} import config\n\n\n"
...
def fork_app(label, folder_path, logger=None):
    ...
    gravyboat_app_path = join(gravyboat.__path__[0], 'apps', label_folder)
    if exists(os.path.join(gravyboat_app_path, 'admin.py')):
        logger.info("Creating admin.py")
        create_file(join(local_app_path, 'admin.py'),
                    "from gravyboat.apps.%s.admin import *  # noqa\n" % label)
    ...
    gravyboat_models_path = join(gravyboat_app_path, 'models.py')
    if exists(gravyboat_models_path):
        logger.info("Creating models.py")
        create_file(
            join(local_app_path, 'models.py'),
            "from gravyboat.apps.%s.models import *  # noqa\n" % label)

        migrations_path = 'migrations'
        source = join(gravyboat_app_path, migrations_path)
        if exists(source):
